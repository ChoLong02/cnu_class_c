...
int main() {
    while(1) {
        ...
        if(menu_num == 1) {          // 햄버거 세트
            empty_idx = empty_order_search();  // 빈 칸 찾기
            ...
            menu_save[empty_idx] = side_name[order_num-1];  // 버거 이름
            price_save[empty_idx] = side_price[order_num-1];
            
            empty_idx = empty_order_search();  // 빈 칸 찾기
            order_num = choice_drink();
            menu_save[empty_idx] = drink_name[order_num-1];  // 버거 이름
